"""
AURORA is an electron transport code.
It is used in IonChem to produce production rates for several ions and excited states.
The production rates are calculated for a narrow energy band, so that we can call them mono-energetic.
The energy resolution coresponds to the one used in ELSPEC.
For other energy grids, one may interpolate these results.
"""
...
for i in length(e_grid)-1:-1:1
    ...
    f = matopen(joinpath(directory_to_process, name_savedir, "Qzt_all_L.mat"))
        ...
        QO2i = read(f, "QO2i")
        QOi  = read(f, "QOi")
        Q_from_O  = read(f, "Q_from_O" )
        Q_from_O2 = read(f, "Q_from_O2")
        Q_from_N2 = read(f, "Q_from_N2")
    close(f)
    using Plots
    plot(Q_from_O[:, 1], h_atm/1e3)
